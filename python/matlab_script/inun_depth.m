...
fdir_output=[fdir_writeout 'InunDepth/' domain];
eval(['mkdir ' fdir_output]);

fdir_output_asc=[fdir_writeout 'InunDepth_ASC/' domain];
...
depmax=load([fdir_result 'Depmax_' num]);
...
alpha(B==255)=0;    
imwrite(rgb,[fdir_output cases{k} '_inundep.png'],'Alpha',alpha)
...
alpha=ones(siz(1),siz(2));
alpha(B==255)=0;    
imwrite(rgb,[fdir_output 'COMB_inundep.png'],'Alpha',alpha)

% write out inundation depth

file_name=[fdir_output_asc 'COMB_inundep.asc'];
fileID=fopen(file_name,'w');
fprintf(fileID,'ncols %d\n',A1(1));
fprintf(fileID,'nrows %d\n',A1(2));
fprintf(fileID,'xllcorner %12.6f\n',A2(1));
