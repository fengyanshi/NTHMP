...
fdir_output=[fdir_writeout 'Vort_wetdry/' domain];
eval(['mkdir ' fdir_output]);

fdir_output_asc=[fdir_writeout 'Vort_wetdry_ASC/' domain];
...
thing=load([fdir_result 'VORmax_' num]);
...
alpha(B==255)=0;    
imwrite(rgb,[fdir_output cases{k} '_vort_wetdry.png'],'Alpha',alpha)

% write out asc
file_name=[fdir_output_asc  cases{k} '_vort_wetdry.asc'];
...
alpha=ones(siz(1),siz(2));
alpha(B==255)=0;    
imwrite(rgb,[fdir_output 'COMB_vort_wetdry.png'],'Alpha',alpha)

% write out inundation depth

file_name=[fdir_output_asc 'COMB_vort_wetdry.asc'];
fileID=fopen(file_name,'w');
fprintf(fileID,'ncols %d\n',A1(1));
fprintf(fileID,'nrows %d\n',A1(2));
fprintf(fileID,'xllcorner %12.6f\n',A2(1));
